...
fn main() -> Result<()> {
    ...
    let module = Module::from_file(store.engine(), "fibonacci.wat")?;
    let instance = Instance::new(&mut store, &module, &[])?;

    // Invoke `fibonacci` export with higher and higher numbers until 
    // we exhaust our fuel
    ...
    for n in 1.. {
        ...
        let output = match fibonacci.call(&mut store, n) {
            Ok(v) => v,
            Err(_) => {
                println!("Exhausted fule computing fib({})", n);
                break;
            }
        };
